...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        scores = X[i].dot(W)
        stable_score = scores - np.max(scores)
        exp_scores = np.exp(stable_score)
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    stable_scores = scores - max_per_row  # For numerical stability

    # Compute the exponential scores and probabilities
