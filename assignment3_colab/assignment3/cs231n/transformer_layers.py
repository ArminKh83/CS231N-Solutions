...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(N, S, self.n_head, self.head_dim).permute(0, 2, 1, 3)
        key = key.view(N, T, self.n_head, self.head_dim).permute(0, 2, 1, 3)
        value = value.view(N, T, self.n_head, self.head_dim).permute(0, 2, 1, 3)

        attention_weights = torch.matmul(query, key.permute(0, 1, 3, 2)) / math.sqrt(self.head_dim)  # (N, H, S, T)

        if attn_mask is not None:
            # Ensure small probabilities in softmax
            attention_weights = attention_weights.masked_fill(attn_mask == 0, float("-inf"))

        attention_weights = self.attn_drop(F.softmax(attention_weights, dim=-1)) @ value
