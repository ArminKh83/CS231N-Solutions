...
def discriminator_loss(logits_real, logits_fake):
    ...
    target_real = torch.ones_like(logits_real).view(-1)
    target_fake = torch.zeros_like(logits_fake).view(-1)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    scores_fake_loss = 0.5 * (scores_fake ** 2).mean()
    scores_real_loss = 0.5 * ((scores_real - 1) ** 2).mean()

    loss = scores_fake_loss + scores_real_loss
