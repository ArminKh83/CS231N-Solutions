...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        encoded_caption = self.positional_encoding(embeded_captions)

        projected_features = self.visual_projection(features).unsqueeze(1)

        o = torch.ones(T, T, device=embeded_captions.device, dtype=embeded_captions.dtype)
        tgt_mask = torch.tril(o)

        features = self.transformer(tgt=embeded_captions,
