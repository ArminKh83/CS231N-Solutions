...
def sim(z_i, z_j):
    ...
    ##############################################################################
    dot_product = torch.matmul(z_i.T, z_j)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sims_k = torch.sum(torch.exp(sims_k / tau))

        up = torch.exp(sim(z_k, z_k_N) / tau)
        l_k = -torch.log(up / sims_k)

        sims_k_N = torch.tensor([sim(z_k_N, z) for z in out[np.arange(2 * N) != k + N]])
        sims_k_N = torch.sum(torch.exp(sims_k_N / tau))

        l_k_N = - torch.log(up / sims_k_N)
...
def compute_sim_matrix(out):
    ...
    normalized_out = out / torch.norm(out, dim=1, keepdim=True)
